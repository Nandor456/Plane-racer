...
namespace repulo_project
{
    internal class GlObject
    {
        ...
        /// 
        internal static GlObject CreateTorus(GL gl, float majorRadius, float minorRadius, int majorSegments = 64, int minorSegments = 16)
        {
            ...
            for (int i = 0; i <= majorSegments; i++)
            {
                ...
                for (int j = 0; j <= minorSegments; j++)
                {
                    ...
                    // Normal (approximate as direction from center of torus ring)
                    float nx = cosPhi * cosTheta;
                    float ny = sinPhi;
                    float nz = cosPhi * sinTheta;
                    vertices.AddRange(new[] { nx, ny, nz });

                    // Color (e.g. green)
                    colors.AddRange(new[] { 0f, 1f, 0f, 1f });
                }
            }
            ...
            return CreateGlObjectFromVertexDescriptions(gl, vertices, colors, indices);
        }
